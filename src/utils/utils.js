 ...
 */
export const e_v = () => {
  const { User: { session: { email_verified }}} = storeProvider.getStore().getState()

  return email_verified == 'yes'
}
...
export const forProfile = async options => {
  let { username, dispatch, invalidUser } = options,
    { data: valid } = await post('/users/is-user-valid', { username })
    const { User: { session: { username: s_username }}} = storeProvider.getStore().getState()
  ...
  if (!valid) {
    ...
    if (username != s_username) {
      ...
      post('/follows/view-profile', { username })
    }
...
export const forGroup = async options => {
  let { grp_id, dispatch, invalidGroup } = options,
    { data: valid } = await post('/groups/is-group-valid', { grp_id })
...
export const insta_notify = async options => {
  ...
  await post('/notifications/notify', {
    to,
    type,
    post_id,
    group_id,
